                    ...
                    <select name="customer_id" id="customer_id"
                        ...
                        @foreach ($customers as $customer)
                            <option value="{{ $customer->id }}"
                                {{ isset($leaseContract) && $leaseContract->customer_id == $customer->id ? 'selected' : '' }}>
                                {{ $customer->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="user_id" id="user_id"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}"
                                {{ isset($leaseContract) && $leaseContract->user_id == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method"
                        ...
                        <option value="creditcard"
                            {{ isset($leaseContract) && $leaseContract->payment_method === 'creditcard' ? 'selected' : '' }}>
                            Creditcard</option>
                        <option value="bank_transfer"
                            {{ isset($leaseContract) && $leaseContract->payment_method === 'bank_transfer' ? 'selected' : '' }}>
                            Bankoverschrijving</option>
                        <option value="paypal"
                            {{ isset($leaseContract) && $leaseContract->payment_method === 'paypal' ? 'selected' : '' }}>
                            PayPal</option>
                        <option value="cash"
                            {{ isset($leaseContract) && $leaseContract->payment_method === 'cash' ? 'selected' : '' }}>
                            Contant</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="notice_period" id="notice_period"
                        ...
                        <option value="maandelijks"
                            {{ isset($leaseContract) && $leaseContract->notice_period === 'maandelijks' ? 'selected' : '' }}>
                            Maandelijks</option>
                        <option value="per kwartaal"
                            {{ isset($leaseContract) && $leaseContract->notice_period === 'per kwartaal' ? 'selected' : '' }}>
                            Per kwartaal</option>
                        <option value="per jaar"
                            {{ isset($leaseContract) && $leaseContract->notice_period === 'per jaar' ? 'selected' : '' }}>
                            Per jaar</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="pending"
                            {{ isset($leaseContract) && $leaseContract->status === 'pending' ? 'selected' : '' }}>In
                            afwachting</option>
                        <option value="active"
                            {{ isset($leaseContract) && $leaseContract->status === 'active' ? 'selected' : '' }}>Actief
                        </option>
                        <option value="terminated"
                            {{ isset($leaseContract) && $leaseContract->status === 'terminated' ? 'selected' : '' }}>
                            Beëindigd</option>
                        <option value="completed"
                            {{ isset($leaseContract) && $leaseContract->status === 'completed' ? 'selected' : '' }}>
                            Voltooid</option>
                    </select>
                </div>
            </div>
                    ...
                    <div class="price-line grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="machine_id[]"
                                class="machine-selector block w-full p-3 border rounded-lg bg-gray-50 focus:ring-yellow-500 focus:border-yellow-500"
                                required>
                        ...
                        <div>
                            ...
                            <input type="number" name="price[]"
                                class="price-input w-full p-3 border rounded-lg bg-gray-200 focus:ring-yellow-500 focus:border-yellow-500"
                                readonly required>
                        </div>
                        <div>
